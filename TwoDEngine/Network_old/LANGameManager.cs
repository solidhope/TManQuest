...
namespace TwoDEngine.Network
{
    public class LANGameManager : GameComponent, NetworkGameManager,GameProtocolListener
    {
        String name;
        String description;
        List<NetworkGamePlayer> players = new List<NetworkGamePlayer>();
        ...
        public LANGameManager(Game game, String name, String description,int maxDataSize=1024, 
            int ttl=1,float introductionFrequency = 2,IPAddress multicastIpAddress = null):base(game)
        {
            this.name = name;
        ...
        {
            Guid guuid = new Guid();
            LANGame game = new LocalLANGame(guuid, name, description);
            games.Add(uuid, game);
            return game;
        }
