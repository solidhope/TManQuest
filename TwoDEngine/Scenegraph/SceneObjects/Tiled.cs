 ...
 * - CF-compatible System.IO.Compression library available at 
 * 
...
namespace Squared.Tiled {
    public delegate void CellVisitor(Vector2 cell);

    internal class Tileset {
        internal class TilePropertyList : Dictionary<string, string> {
        }
        ...
        internal static Tileset Load (XmlReader reader) {
            // requires a content manager be avalbale
        ...
        public Texture2D Texture {
            ...
            set {
                ...
                NumberOfTiles = ((int)(_Texture.Width/TileWidth))*((int)(_Texture.Height/TileHeight));
            }
        }
    ...
    internal class Layer
    {
        ...
        internal static Layer Load(XmlReader reader)
        {
            ...
            if (reader.GetAttribute("visible") != null)
                result.Visible = float.Parse(reader.GetAttribute("visible"));
        ...
        public void ForEveryCell(CellVisitor visitor)
        {
            Vector2 cellPos;
    ...
    internal class ObjectGroup
    {
        ...
        public void Draw(TileMap result, SpriteBatch batch, Rectangle rectangle, Vector2 viewportPosition)
        {
    ...
    internal class TileMap
    {
        ...
        public Dictionary<string, Layer> layerDictionary = new Dictionary<string, Layer>();
        public SortedList<string, ObjectGroup> ObjectGroups = new SortedList<string, ObjectGroup>();
        public SortedList<string, string> Properties = new SortedList<string, string>();
        public int Width, Height;
        public int TileWidth, TileHeight;

        public static TileMap Load (string filename, ContentManager content) {
                                    ...
                                    case "layer":
                                        {
                                            using (var st = reader.ReadSubtree())
                                            {
                                                ...
                                                result.layerDictionary.Add(layer.Name, layer);
        ...
        public Tileset GetTilesetForTile(int tileNum)
        {
            foreach (Tileset tileset in Tilesets.Values)
            {
                ...
                //Console.WriteLine("First tile: " + tileset.FirstTileID);
                //Console.WriteLine("TileCount: " + tileset.NumberOfTiles);
                if ((tileNum>=tileset.FirstTileID)&&(tileNum<(tileset.FirstTileID+tileset.NumberOfTiles))){
                    return tileset;
                } 
            }
            throw new InvalidDataException("Tile number " + tileNum + " not in any tileset currently loaded.");
        }
    }
}
